...
Object.assign(I18N_PATCH.pt, {
  ...
  mockForwardLast: 'Último encaminhamento mock',
  mockStatusPending: 'Pendências para encaminhar',
  mockStatusReady: 'Sem pendências'
});
Object.assign(I18N_PATCH.en, {
  ...
  mockForwardLast: 'Last mock forwarding',
  mockStatusPending: 'Pending records to forward',
  mockStatusReady: 'No pending records'
});
...
function setupDeviceCompatibility() {
  const mode = state.device?.device_type || window.RecycleUtils?.detectDeviceType?.() || 'desktop';
  ...
  if (badge) {
    ...
    const dict = LANGUAGES[state.lang] || LANGUAGES.pt;
    badge.textContent = `${dict.deviceMode || 'Mode'}: ${labels[mode]} (${state.device?.device_name || mode})`;
  }
...
function createForm(type, container) {
  ...
  const performSave = () => {
    ...
    const result = window.RecycleTransactions?.saveTransaction
      ? window.RecycleTransactions.saveTransaction({
        data: payload,
        type,
    ...
    rememberFieldValues(type, payload);
  ...
  state.transactions
    ...
    .forEach((r) => {
      ...
      if (isNarrowScreen() && window.RecycleClients?.renderMobileClientRow) {
        ...
        tr.innerHTML = `<td>${r.clientId}</td><td>${r.clientName || ''}</td><td>${r.phone || ''}</td><td>${r.entryDate || ''}</td><td>${r.transactionCode || ''}</td><td>${formatMoney(r.total)}</td><td>${translateTransactionType(r.type)}</td><td>${(r.created_by_device_name || r.created_by_device_id || '-')}</td><td><button class="printRow" data-code="${r.transactionCode}">${t('printLabel')}</button> <button class="editRow">${t('edit')}</button> <button class="printClientHistoryRow">${t('printClientHistory')}</button> <button class="deleteClientRow">${t('deleteClient')}</button></td>`;
      }
...
function renderSystemInfo() {
  ...
  const summary = window.RecycleSync?.getSyncSummary ? window.RecycleSync.getSyncSummary() : { pending: 0, synced: 0, error: 0, last_synced_at: null };
  const setText = (id, value) => {
    const el = document.getElementById(id);
    if (el) el.textContent = value || '-';
  };

  setText('systemDeviceId', device.device_id || '-');
  setText('systemDeviceName', device.device_name || '-');
  setText('systemDeviceType', device.device_type || '-');
  setText('systemLastSyncTime', summary.last_synced_at || '-');
  setText('systemMockForwardTime', summary.last_synced_at || '-');
  ...
  const badge = document.getElementById('mockForwardStatusBadge');
  if (badge) {
    badge.classList.remove('pending', 'ready');
    if (pending > 0) {
      badge.classList.add('pending');
      badge.textContent = `${t('mockStatusPending')}: ${pending}`;
    } else {
      badge.classList.add('ready');
      badge.textContent = t('mockStatusReady');
    }
  }
}
