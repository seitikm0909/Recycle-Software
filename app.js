...
function renderChart(purchaseTotal, saleTotal) {
  ...
  if (mode === 'bar') {
    ...
    ['Compra', 'Venda'].forEach((name, idx) => {
      const x = 180 + idx * 280;
      const h = (values[idx] / max) * 220;
      ctx.fillStyle = colors[idx];
      ctx.fillRect(x, 300 - h, 120, h);
      ctx.fillStyle = '#111';
      ctx.fillText(`${name}: ${values[idx].toFixed(2)}`, x, 325);
    });
  } else {
    ...
    [purchaseTotal, saleTotal].forEach((value, idx) => {
      ...
      ctx.beginPath();
      ctx.moveTo(350, 190);
      ctx.arc(350, 190, 150, start, start + angle);
      ctx.closePath();
      ctx.fillStyle = colors[idx];
      ctx.fill();
      start += angle;
    ...
    ctx.fillStyle = '#111';
    ctx.fillText(`Compra: ${purchaseTotal.toFixed(2)}`, 560, 160);
    ctx.fillText(`Venda: ${saleTotal.toFixed(2)}`, 560, 190);
  }
}
